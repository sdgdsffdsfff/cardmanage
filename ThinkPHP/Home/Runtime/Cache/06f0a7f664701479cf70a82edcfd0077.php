		...
		<div id="main">
			...
			<tbody>
				<tr style="background-color:#E5E5E5;font-weight:bold;height:15px;">
					<th scope="col">序号</th><th scope="col">帐号</th><th scope="col">密码</th><th scope="col">状态</th><td>类别</td><th scope="col">联系电话</th><th scope="col">上级代理</th><th scope="col">操作</th>
				</tr>
				<?php
 foreach($accountdata as $key =>$value) { echo '<pre>'; echo '</pre>'; $status=''; if($value['status']==1) { $status='开通'; } else { $status="锁定"; } $type=''; switch (2) { case 1: $type="总管理员"; break; case 2: $type="一级代理"; break; default: $type="二级代理"; break; } echo '<tr><td>'.$key.'</td><td>'.$value['loginname'].'</td><td>******</td><td>'.$status.'</td><td>'.$type.'</td><td>'.$value['tele'].'</td><td>'.$_SESSION['loginname'].'</td><td><a href="__APP__/authority/updateaccdata?id='.$value['id'].'">【修改】</a></td></tr>'; } ?>
