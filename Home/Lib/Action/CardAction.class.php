...
class CardAction extends CommonAction {
    ...
    public function index() {
        ...
        if (!$info)
            exit('<p style="color:#666;padding:10px;font-size:10pt;">暂无制卡权限，请联系上级管理员！</p>');
    ...
    public function add() {
        if ($this->isAjax()) {
            ...
            if ($end < $start)
                $this->ajaxReturn(array('status' => 'error', 'message' => '结束账号不得小于起始账号！'));
            if (strlen($end) > $numlen || strlen($start) > $numlen)
                $this->ajaxReturn(array('status' => 'error', 'message' => '起始账号或者结束账号都不得大于' . $numlen . '位！'));
