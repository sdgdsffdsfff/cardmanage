...
class BlackcallednumAction extends  CommonAction {
  ...
  function index()
  {
        ...
        if($this->isPost()){//搜索
        	...
        	$where.='areacode like \'%'.$_POST['num'].'%\'';
        	if(isset($_POST['starttime']) && !empty($_POST['starttime'])) $where.=' AND addtime >'.$starttime;
        	if(isset($_POST['endtime']) && !empty($_POST['endtime'])) $where.=' AND addtime <'.$endtime;
        }
        $count=$model->query('select count(*) from cb_blacklist '.$where);
        $count=$count[0]['count'];
        $header="条黑名单信息";
        $pagesize=17;
        ...
        $sql="select * from cb_blacklist ".$where;
     ...
     #删除用户操作
      function delete()
      {
        if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"])=="xmlhttprequest"){
             ...
             $model=M('numberpool');
             $sqlstr="delete from cb_blacklist where id = ".$_POST['id'];
 ...
 #跳转到添加
  ...
  {   
    if($this->isAjax()){
            if (empty($_POST['blacknum'])) {
               $data['status']="failed";
               $data['message']="添加失败。黑名单号码不为空！";
               $this->ajaxReturn($data);
            }
             $blacknum=trim($_POST['blacknum']);   
             $model=M('numberpool');
             $sqlstr="insert into cb_blacklist (areacode,addtime) values('".$blacknum."','".time()."')";
             $addstatus = $model->execute($sqlstr);
        if ($addstatus) {
            $data['status']="success";
            $data['message']="添加成功。";
        }
    ...
    function check_telnum($post)
     ...
     function checkcalltransfernum($calltransfernum)
     {
          ...
          $model=M('numberpool');
          $sqlstr="selct count(*) from cb_blacklist where num = '".$calltransfernum."'";
      ...
      #批量导入常用联系人
          ...
          if ($_SERVER['REQUEST_METHOD'] == 'POST')   //是否是post上传的文件
              {            
                ...
                //是否存在文件
                {
                  ...
                  $this->assign('jumpurl','__APP__/Blackcallednum/bulkadd');                   
                  $this->display('jump'); 
                  exit();                         
                }
                else
       ...
       function bulkadddata($data)
       {
           ...
           foreach ($data as $key => $value) {   
                if ($value!=''&&$value!=null) {
                    $sqlstr="insert into cb_blacklist (areacode,addtime) values('".$value."','".time()."')";         
                    $status=$model->execute($sqlstr);
                 if (!$status) {
                       $addresult[$key]="黑名单号码".$value."添加失败";        
                  }
                  else
                  {
                       $addresult[$key]="黑名单号码".$value."添加成功";
                  }   
                  }  
       ...
       {      
            ...
            foreach ($data as $key => $value) {
                   ...
                   {
                           $returndata[$key]="黑名单号码".$value."在该文件中重复，请查证。";
                           unset($contactdata[$key]);
                   }
        ...
        function download()
        {
             ...
             else
             {
              $file_name=$this->formfile($numdata,2);
                  ...
                  Header("Content-Disposition: attachment; filename="."黑名单".$file_name); 
                  $buffer=1024; 
                  $file_count=0; 
        ...
        function formfile($numdata,$type)
        {
          ...
          else 
          {
            ...
            $objExcel->getProperties()->setSubject("黑名单号码");  
            $objExcel->getProperties()->setDescription("黑名单号码");  
           ...
           foreach ($numdata as $key => $value) { 
            ...
            header('Content-Disposition: attachment;filename="'.iconv('utf-8', 'gbk', '黑名单').date('Y-m-d',$timestamp).'.xls"');  
    ...
    function querynum()
    {
        ...
        {     
              $num=trim($_POST['num']);
              $count=$model->query("select count(*) from cb_blacklist where areacode like'%".$num."%'");
              $count=$count[0]['count'];
              $header="条黑名单信息";
              $pagesize=17;
              $Page    = new Page($count,$pagesize);// 实例化分页类 传入总记录数和每页显示的记录数
