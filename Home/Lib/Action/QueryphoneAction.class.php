    ...
    function updatephonedata() {
        if (isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"]) == "xmlhttprequest") {
            ...
            if ($status) {
                ...
                //修改状态
                if ($userinfo['state'] != $data['state'])
                    $operdetail.='　是否禁用改为' . ($data['state'] ? '否' : '是');
                //修改密码
                if ($userinfo['loginpwd'] != $data['loginpwd'])
                    $operdetail.='　密码从' . $userinfo['loginpwd'] . '变为' . $data['loginpwd'];
                //修改余额
                if ($userinfo['banlance'] != $data['banlance'])
                    $operdetail.='　余额从' . $userinfo['banlance'] . '变为' . $data['banlance'];
                //截止日期
                if (date('Y-m-d', trim($userinfo['expirydate'])) != date('Y-m-d', trim($data['expirydate'])))
                    $operdetail.='　截止日期从' . date('Y-m-d', $userinfo['expirydate']) . '变为' . date('Y-m-d', $data['expirydate']);
                //备注
                if ($userinfo['remark'] != $data['remark'])
                    $operdetail.='　备注从' . $userinfo['remark'] . '变为' . $data['remark'];
                $opertype = "修改用户信息";
                $data['status'] = "success";
                $data['message'] = "修改成功";
            }
