	...
	function  updateacc()
	{
		...
		if(!$this->chk_delspau())
		{
	    	...
	    	$this->assign('
                ...
                $up_powersataus = $power_model->where('accountid='.$_POST['id'])->save(
                ...
                $reauthoritydata);
                  ...
                  foreach ($array  as $value) {
                  if($powerdata[0][$value]!=$authoritydata[$val
                  ue])
                  ...
                  #淇敼涔嬪墠鏉冮檺 淇敼涔嬪悗鐨勬潈闄�
                  function validatorsubauthority($powerdata,$author
                  itydata,$array)
                  {
                  //print_r($powerdata);
                  $reauthoritydata=array();
