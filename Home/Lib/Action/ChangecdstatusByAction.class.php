...
class ChangecdstatusByAction extends  CommonAction {
	...
	function changecdstatus() {
		...
		$sql = 'select cardnum from cb_cards where length(cardnum) = 6 order by cardnum desc limit 1';
		...
		if ($type == 1) {
			...
			$sql = "UPDATE cb_cards SET  status=1 ,openway=2,opentime=" . time() . "  where cardnum >= trim(to_char(" . $startcardnum . ",'000000')) and cardnum <= trim(to_char(" . $stopcardnum . ", '000000')) and length(cardnum) = 6";
			//返回受影响的条数
			...
			$sql = "UPDATE cb_cards SET  status=3,locktime=" . time() . "  where cardnum >= trim(to_char(" . $startcardnum . ",'000000')) and cardnum <= trim(to_char(" . $stopcardnum . ", '000000')) and length(cardnum) = 6";
			//返回受影响的条数
			$status = $card_model -> execute($sql);
			//$status = $card_model->where('cardnum>='.$startcardnum.'and cardnum<='.$stopcardnum)->save($data);
		}
