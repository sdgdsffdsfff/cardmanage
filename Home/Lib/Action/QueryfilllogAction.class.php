...
class QueryfilllogAction extends  CommonAction {
  ...
  {    
        ...
        if($this->isPost() || isset($_REQUEST['starttime'])){
        	...
        	if(isset($_REQUEST['cardno']) && !empty($_REQUEST['cardno']))$where.=" AND cardno like '%{$_REQUEST['cardno']}%'";
        	if(isset($_REQUEST['phonenum']) && !empty($_REQUEST['phonenum']))$where.=" AND phonenum like '%{$_REQUEST['phonenum']}%'";        	
        }
  ...
  function querylogbytime()
  {
      if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"])=="xmlhttprequest"){
          ...
          $log_model=M('fill_log');
          $where="filldate>=".$starttime." and filldate<=".$endtime;              
          if(isset($_POST['cardno']) && !empty($_POST['cardno']))$where.=" AND cardno like '%{$_POST['cardno']}%'";
          if(isset($_POST['phonenum']) && !empty($_POST['phonenum']))$where.=" AND phonenum like '%{$_POST['phonenum']}%'";
