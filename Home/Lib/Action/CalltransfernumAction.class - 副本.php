...
class CalltransfernumAction extends  CommonAction {
 ...
 #跳转到添加
  ...
  #添
  function addcalltransfernum()
  {   
    if(isset($_SERVER["HTTP_X_REQUESTED_WITH"]) && strtolower($_SERVER["HTTP_X_REQUESTED_WITH"])=="xmlhttprequest"){
          
            if (empty($_POST['calltransfernum'])) {
               $data['status']="failed";
               $data['message']="添加失败。呼转号码不为空！";
               $this->ajaxReturn($data);
            }
           ...
           if (!$telstatus) {
             # code..
              $data['status']='failed';
              $data['message']="呼转号码格式不正确";
              $this->ajaxReturn($data,'json');
           }*/

            /*
            $this->checkcalltransfernum($_POST['calltransfernum']);
            */
             $transfernum=trim($_POST['calltransfernum']);   
             $model=M('numberpool',"cb_",'DB_CONFIG2');
             $sqlstr="insert into cb_numberpool values('".$transfernum."')";
      ...
      #批量导入常用联系人
       ...
       function bulkadddata($data)
       {
           ...
           foreach ($data as $key => $value) {     
                 $sqlstr="insert into cb_numberpool values('".$value."')";         
                 $status=$model->execute($sqlstr);
                 if (!$status) {
       ...
       {      
        ...
        function formfile($numdata,$type)
        {
          ...
          else 
          {
            ...
            $objExcel->getProperties()->setLastModifiedBy("赵兴壮");  
           ...
           foreach ($numdata as $key => $value) { 
            ...
            $timestamp = time();  
                ...
                header('Content-Disposition: attachment;filename="呼转号码'.$timestamp.'.xls"');  
