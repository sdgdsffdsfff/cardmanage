		...
		<table rules="all" id="MyGridView"
			...
			<tbody>
				...
				<tr
					...
					<?php
 if($_SESSION['power']==1) { echo '<th scope="col" style="width:60px">全选<input type="checkbox" name="checkall" value="all"/></th>'; } ?>
					<th scope="col">卡号</th>
					<th scope="col">密码</th>
					<th scope="col">电话</th>
				...
				<?php
 if (!empty($list)) { foreach ($list as $key => $cardvalue) { echo '<tr id="tr'.$cardvalue['id'].'" style="height:25px;" align="center">'; if($_SESSION['power']==1) { echo '<td><input style="width:50px" type="checkbox" name="check[]" class="check" value="'.$cardvalue['id'].'"/></td>'; } echo '<td align="center" width="70px">'.$cardvalue['cardnum'].'</td>
