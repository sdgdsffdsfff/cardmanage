...
<div class="head-dark-box">
   ...
   <form name="Form" method="post"  target="main">
       代理商：<select  id="id"  onchange="window.location='__APP__/Ratemanage/qubyacc?id='+document.Form.id.options[selectedIndex].value">
			...
			<?php  foreach ($accountdata as $value) { if($value['id']!=$id){ if ($value['id']==$ownid) { echo '<option value ="'.$value['id'].'" selected >'.$value['loginname'].'</option>'; } else { if($value['power']!=1){ echo '<option value ="'.$value['id'].'">'.$value['loginname'].'</option>'; } } } } ?>
		</select><span style="color:red">&nbsp;&nbsp;&nbsp;&nbsp;如果代理商没有设定费率,则执行管理员费率。</span>
	 </form>
...
<table rules="all" id="MyGridView" style="border-color:Black;border-width:1px;border-style:solid;font-size:10pt;height:10px;width:90%;border-collapse:collapse;" cellpadding="3" cellspacing="0" align="Center" border="1">
	<tbody>
		...
		<tr>
						...
						<td colspan="7">
						   <a href="__APP__/Ratemanage/add?ownid=<?php echo $ownid?>">添加费率</a> 
						   <a href="__APP__/Ratemanage/defaultrate?ownid=<?php echo $ownid?>">默认费率</a>
						</td>
		...
		<tr style="background-color:#E5E5E5;font-weight:bold;height:15px;">
		   ...
		   <?php  foreach ($feedata as $key => $pervalue) { if(($pervalue['starttime']=="0000")&&($pervalue['endtime']=="0000")){ } else { echo '<tr id="tr'.$pervalue['id'].'">
                ...
                <?php
 if (empty($feedata)) { echo "<tr><td colspan='6' align='center'  style='color:red;'> 该代理商没有设置费率,执行管理员费率</td></tr>"; } if (!empty($defaultfeedata)) { echo '<tr><td colspan="6" align="center" style="color:red;">如果不在以上时间段执行默认费率 '.$defaultfeedata[0]['fee'].'元</td></tr>'; } else { echo "<tr><td colspan='6' align='center' style='color:red'> 您没有设置默认费率，执行管理员默认费率。</td></tr>"; } ?>
